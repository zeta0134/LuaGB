...
filebrowser.refresh_items = function()
  local pwd_items = luagb.filesystem.getDirectoryItems(filebrowser.pwd)
  filebrowser.items = {}
  filebrowser.cursor_pos = 0
  filebrowser.scroll_pos = 0
  ...
  for _, path in pairs(pwd_items) do
    if luagb.filesystem.isDirectory(filebrowser.pwd .. path) then
  ...
  for _, path in pairs(pwd_items) do
    if not luagb.filesystem.isDirectory(filebrowser.pwd .. path) then
      table.insert(filebrowser.items, path)
    end
  end
end
...
filebrowser.select_at_cursor = function()
  ...
  if luagb.filesystem.isDirectory(filebrowser.pwd .. cursor_item) then
...
filebrowser.open_save_directory = function(button)
  love.system.openURL("file://"..luagb.filesystem.getSaveDirectory())
end
