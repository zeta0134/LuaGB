luagb = {
  require = love.require
}

luagb.filesystem = love.filesystem
local bit32 = bit
local filebrowser = luagb.require("filebrowser")
local Gameboy = luagb.require("gameboy/init")
...
function LuaGB:save_ram()
  ...
  if luagb.filesystem.write(filename, save_data) then
...
function LuaGB:load_ram()
  local filename = "saves/" .. self.game_filename .. ".sav"
  local file_data, size = luagb.filesystem.read(filename)
...
function LuaGB:save_state(number)
  ...
  if luagb.filesystem.write(filename, state_string) then
...
function LuaGB.dump_audio(buffer)
  ...
  luagb.filesystem.append("audiodump.raw", output, 32768 * 2)
  ]]
end
...
function LuaGB:load_game(game_path)
  ...
  local file_data, size = luagb.filesystem.read(game_path)
  if file_data then
...
function love.load(args)
  ...
  luagb.filesystem.createDirectory("games")
  luagb.filesystem.createDirectory("states")
  luagb.filesystem.createDirectory("saves")
  ...
  filebrowser.is_directory = luagb.filesystem.isDirectory
  filebrowser.get_directory_items = luagb.filesystem.getDirectoryItems
...
action_keys.a = function()
  if LuaGB.audio_dump_running then
    ...
    luagb.filesystem.remove("audiodump.raw")
